#!/usr/bin/env python3
"""
Pump Rider — Trader v4
========================
v4 adds:
  - ✅ RESCANNING while holding: every run, re-scans the market
  - ✅ SWITCH: if a new candidate is 20+ points better than our position's score, close and enter
  - ✅ STAGNATION EXIT: if position is between -1% and +1% after 1h+, close and look elsewhere
  - ✅ PARTIAL TAKE PROFIT (warmup): if PnL >= +2.5% but momentum 5m is peaking, lock partial gain
  - ✅ FULL rescann embedded in trader.py (no dependency on shell ordering)
"""
...
BINANCE_MAKER_FEE = 0.001      # 0.1% spot fee
BINANCE_TAKER_FEE = 0.001

SWITCH_SCORE_THRESHOLD = 20    # new candidate must beat current score by this much
STAGNATION_MIN_ELAPSED = 3600  # 1 hour before we consider stagnation exit
STAGNATION_BAND = 1.0          # +/-1% PnL band = stagnation
PARTIAL_TP_PCT = 2.5           # lock gain at +2.5%
MAX_HOLD_SECONDS = 7200        # 2h hard cap

# ─── BTC mood filter ──────────────────────────────────────────────────────
BTC_MOOD_CRASH_THRESHOLD = -2.0  # skip trading if BTC dropped this much in 4h
BTC_MOOD_BAD_THRESHOLD = -1.0    # add penalty if BTC is this negative
BTC_1H_CANDLES_TO_CHECK = 4      # look at last 4 hourly candles

# ─── Scanner thresholds (duplicated for self-contained re-scann) ──────────
...
# ─── Shared helpers (same logic as scanner.py) ─────────────────────────
...
def apply_btc_penalty(candidates, btc_mood):
    """Apply BTC mood penalty to all candidates and filter hard-blocked ones."""
    if not candidates:
        return candidates
    if btc_mood["hard_block"]:
        return []  # no trading during BTC crash/freefall
...
def check_btc_mood():
    """
    Fetch last 4 BTC hourly candles and determine market mood.
    Returns dict with btc_price, 4h_change_pct, mood_label, and a penalty/override flag.
    """
    try:
        url = "https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1h&limit=5"
        data = fetch_json(url)
        if len(data) < 2:
            return {"btc_price": None, "4h_change_pct": 0, "mood": "unknown", "penalty": 0, "hard_block": False}
        ...
        if change_pct <= BTC_MOOD_CRASH_THRESHOLD:
            mood = "crash"
            hard_block = True  # don't enter new positions
            penalty = -50
            ...
            mood = "bullish"
            penalty = 0  # no penalty, green light

        # Count red 1h candles
...
def compute_pnl(ap, prices):
    """Compute realistic PnL with fees."""
    if not prices:
        return None, 0
    ...
    if ap["side"] == "UP":
        ...
        raw_pnl_pct = (entry_ask - exit_price) / entry_ask * 100
    fee_pct = (BINANCE_MAKER_FEE + BINANCE_TAKER_FEE) * 100
    pnl_pct = raw_pnl_pct - fee_pct
    # Mid-based trailing reference
    mid_pnl = (prices["mid"] - ap["entry_price"]) / ap["entry_price"] * 100 if ap["side"] == "UP" else 0
    return pnl_pct, mid_pnl


# ─── Decision: check current position ──────────────────────────────────

def check_position(state, fresh_candidates):
    """
    v4: Check current position with expanded exit reasons.
    Returns (action_dict, switched_to_candidate).
    """
    ...
    highest_price = max(ap.get("highest_price", ap["entry_price"]), prices["mid"])
    stop_pct = ap.get("stop_pct", -4.0)

    # Tighten stop as profit grows
    if mid_pnl >= 5: stop_pct = -2.0
    elif mid_pnl >= 3: stop_pct = -3.0
    ...
    # 4) v4: Stagnation exit (between -1% and +1% after 1h)
    elif elapsed > STAGNATION_MIN_ELAPSED and abs(pnl_pct) <= STAGNATION_BAND:
        # Check if there's something better to switch to
        if fresh_candidates and fresh_candidates[0]["score"] >= MIN_SCORE_TO_TRADE:
            ...
            if best_new_score > current_entry_score + SWITCH_SCORE_THRESHOLD:
                ...
                # Stagnating but nothing better — keep holding
                should_exit = False
                reason = f"stagnation_but_no_better_option"

    # 5) v4: Switch to better candidate (even if not stagnating)
    if not should_exit and fresh_candidates and fresh_candidates[0]["score"] >= MIN_SCORE_TO_TRADE:
        current_entry_score = ap.get("entry_score", 0)
        best_new_score = fresh_candidates[0]["score"]
        # Only switch if the new one is significantly better AND our position isn't doing well
    ...
    # 6) v4: Partial TP — +2.5% with fading momentum
    if not should_exit and pnl_pct >= PARTIAL_TP_PCT:
        # Check 5m momentum of current position
        m5 = check_5m_momentum(ap["symbol"])
        if m5 and not m5["last_5m_green"]:
            # Price went up 2.5% but last 5m candle is red — take partial or full
            should_exit = True
            reason = f"partial_take_profit_{pnl_pct}%_5m_fading"
...
def close_position(state, pos_check, reason_suffix=""):
    """Paper-close the active position and log it."""
    ap = state["active_position"]
    entry = ap["entry_price"]
    exit_price = pos_check["bid"]
...
def open_position(state, candidate):
    """Paper-enter a new position from a candidate."""
...
def check_position_btc_mood(state, btc_mood):
    """Check if BTC mood suggests we should close current position early."""
    if not state.get("active_position"):
        return None
    # If BTC is in crash mode, consider exiting even if our coin is fine
    if btc_mood["hard_block"]:
        elapsed = time.time() - state["active_position"].get("opened_at_unix", 0)
        if elapsed > 600:  # at least 10 min into the position
            return {"exit_reason": "btc_mood_crash"}
    return None
...
def execute():
    ...
    # Check BTC mood first — market context
    btc_mood = check_btc_mood()
    result["btc_mood"] = btc_mood

    # Always run scanner (fresh data every run)
    fresh_candidates = run_scanner()
    scanner_report = fresh_candidates[:3]  # top 3 for report

    # Apply BTC mood penalty to candidates
    fresh_candidates = apply_btc_penalty(fresh_candidates, btc_mood)

    # Also save scan_latest.json for external consistency
    scan_output = {
        "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "candidates": fresh_candidates,
        "scanned_count": 100,
        "strategy": "v4_embedded_scanner",
        "btc_mood": btc_mood,
    }
    ...
    result["btc_penalty_applied"] = btc_mood["penalty"] != 0

    # ── Check if BTC crash should force-close position ────────────────
    btc_exit = check_position_btc_mood(state, btc_mood)
    if btc_exit:
        # Force close the position due to BTC crash
        ap = state["active_position"]
        prices = get_spread_prices(ap["symbol"])
        if prices:
            pnl_pct, _ = compute_pnl(ap, prices)
            pos_check = {
                "action": "exit",
                "reason": btc_exit["exit_reason"],
                "bid": prices["bid"],
                "ask": prices["ask"],
                "mid": prices["mid"],
                "pnl_pct": round(pnl_pct, 2),
            ...
            # Don't re-enter after BTC crash exit — hard_block prevents it
    ...
    if state.get("active_position"):
        ...
        if pos_check["action"] == "exit":
            ...
            # If we exited to switch to a better candidate, enter it now
            if switch_target:
                entry_info = open_position(state, switch_target)
                result["switch_entered"] = entry_info

        # Update highest price in state
    ...
    # ── If no position (or just closed), try to enter ──────────────────
    if not state.get("active_position"):
        if fresh_candidates and fresh_candidates[0]["score"] >= MIN_SCORE_TO_TRADE:
    ...
    if ap and pos_check:
        long_signal = {
            ...
            "5m_momentum": check_5m_momentum(ap["symbol"]) if ap["symbol"] else None,
            "stop_pct": pos_check.get("stop_pct"),
        }
    shared_signals = {
        "long_position": long_signal,
    ...
    try:
        ...
        SHARED_SIGNALS_PATH.write_text(json.dumps(shared_signals, indent=2))
    except Exception:
        pass
